...
namespace Drastic.PushNotification
{
    ...
    public class PushNotificationManager : NSObject, IPushNotification, IUNUserNotificationCenterDelegate
    {
        ...
        public async void RemoveNotification(int id)
        {
            ...
            {
                var scheduledNotifications = UIApplication.SharedApplication.ScheduledLocalNotifications.Where(u => (u.UserInfo.ContainsKey(this.notificationIdKey) && u.UserInfo[this.notificationIdKey].Equals($"{id}")) || (u.UserInfo.ContainsKey(this.apsNotificationIdKey) && u.UserInfo[this.apsNotificationIdKey].Equals($"{id}")));
        ...
        public async void RemoveNotification(string tag, int id)
        {
            ...
            else
            {
                ...
                {
                    var scheduledNotifications = UIApplication.SharedApplication.ScheduledLocalNotifications.Where(u => (u.UserInfo.ContainsKey(this.notificationIdKey) && u.UserInfo[this.notificationIdKey].Equals($"{id}") && u.UserInfo.ContainsKey(this.notificationTagKey) && u.UserInfo[this.notificationIdKey].Equals(tag)) || (u.UserInfo.ContainsKey(this.apsNotificationIdKey) && u.UserInfo[this.apsNotificationIdKey].Equals($"{id}") && u.UserInfo.ContainsKey(this.apsNotificationTagKey) && u.UserInfo[this.apsNotificationIdKey].Equals($"{tag}")));
                    foreach (var notification in scheduledNotifications)
                    {
                        UIApplication.SharedApplication.CancelLocalNotification(notification);
                    }
